...
N4_gre_cf_vdps <- read.csv("./IRC740H_visit1_spiral_cf/vdp_analysis_results_October2024/N4_combined_vdps.csv")
FA_gre_cf_vdps <- read.csv("./IRC740H_visit1_spiral_cf/vdp_analysis_results_October2024/FA_combined_vdps.csv")
...
vdpn4_bxp_nop <- ggpaired(N4_data_long, x = "AnalysisMethod", y = "VDP", fill = "AnalysisMethod",
                        ...
                        legend = "none", xlab = "") +
  ylab(ylabel_n4) +
  geom_point(aes(color = "black"), shape = 19, size = 2) +
...
##############################################
ylabel_fa <- expression(bold(VDP[FA] * "(%)")) # Spiral
