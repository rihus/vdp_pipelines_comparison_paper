...
N4_gre_h_vdps <- read.csv("./age_matched_spiral_healthy/vdp_analysis_results_September2024/N4_combined_vdps.csv")
FA_gre_h_vdps <- read.csv("./age_matched_spiral_healthy/vdp_analysis_results_September2024/FA_combined_vdps.csv")
# ##Reshape Data to Long Format for ggplot2
N4_data_long <- N4_gre_h_vdps %>%
  pivot_longer(cols = -Subject_id, names_to = "AnalysisMethod", values_to = "VDP")
FA_data_long <- FA_gre_h_vdps %>%
  pivot_longer(cols = -Subject_id, names_to = "AnalysisMethod", values_to = "VDP")
##Set the order the order of boxes in plots
N4_data_long$AnalysisMethod <- factor(N4_data_long$AnalysisMethod, levels = plt_order)
FA_data_long$AnalysisMethod <- factor(FA_data_long$AnalysisMethod, levels = plt_order)
##############################################
ylabel_n4 <- expression(bold(VDP[Spiral-N4] * "(%)"))
# #Box plots with p-values VDP - N4
vdpn4_bxp_nop <- ggpaired(N4_data_long, x = "AnalysisMethod", y = "VDP", fill = "AnalysisMethod",
...
##############################################
ylabel_fa <- expression(bold(VDP[Spiral-FA] * "(%)"))
# #Box plots with p-values VDP - FA
vdpfa_bxp_nop <- ggpaired(FA_data_long, x = "AnalysisMethod", y = "VDP", fill = "AnalysisMethod",
                        palette = c("#999999","#E69F00","#CC79A7","#F0E442","#56B4E9","#009E73"),
                        id = "Subject_id", width = 0.5,
...
ggsave("./zR_plots_4abs/spir_healthy_vdps_cbxps_N4.png", plot = vdpn4_bxp_nop, width = 4.5, height = 3.7, dpi = 300)
ggsave("./zR_plots_4abs/spir_healthy_vdps_cbxps_FA.png", plot = vdpfa_bxp_nop, width = 4.5, height = 3.7, dpi = 300)
