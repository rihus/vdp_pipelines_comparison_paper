...
FA_cf_j <- FA_cf_sensitivity
FA_cf_j[,-1] <- FA_cf_sensitivity[,-1] + FA_cf_specificity[,-1] - 1

N4_cf_j <- N4_cf_sensitivity
N4_cf_j[,-1] <- N4_cf_sensitivity[,-1] + N4_cf_specificity[,-1] - 1

# Youdens_J
# #Reshape Data to Long Format for ggplot2
FA_cf_long <- FA_cf_j %>%
  pivot_longer(cols = -Subject_id, names_to = "Analysis_Method", values_to = "Youdens_J") %>%
  filter(!is.na(Youdens_J)) %>%
...
# FA_cf_long$Youdens_J <- FA_cf_long$Youdens_J * 100

# N4_cf_long$Youdens_J <- N4_cf_long$Youdens_J * 100

##############################BAR Plots and statistical tests###################
#########################FA
cf_fa_bar <- create_barplot(FA_cf_long, "Analysis_Method", "Youdens_J",
               expression(bold("Youden's J"[FA]* " " *"(%)")),
               "Analysis Method", c(0, 1.05), cbPalette, "Subject_id", plt_order)
friedman.test(Youdens_J ~ Analysis_Method | Subject_id, data = FA_cf_long)
frdAllPairsNemenyiTest(Youdens_J ~ Analysis_Method | Subject_id, data = FA_cf_long)

#########################N4: expression(bold("Mean FP VDP"[N4]* " " *"(%)"))
cf_n4_bar <- create_barplot(N4_cf_long, "Analysis_Method", "Youdens_J",
                            expression(bold("Youden's J"[N4]* " " *"(%)")),
               "Analysis Method", c(0, 1.05), cbPalette, "Subject_id", plt_order)
friedman.test(Youdens_J ~ Analysis_Method | Subject_id, data = N4_cf_long)
frdAllPairsNemenyiTest(Youdens_J ~ Analysis_Method | Subject_id, data = N4_cf_long)

# Save the plot as a png file in the specified directory
ggsave("./zR_plots_4ppr/cf_youdensJ_barplt_FA.png", plot = cf_fa_bar, width = 4.5, height = 3.7, dpi = 300)
ggsave("./zR_plots_4ppr/cf_youdensJ_barplt_N4.png", plot = cf_n4_bar, width = 4.5, height = 3.7, dpi = 300)
